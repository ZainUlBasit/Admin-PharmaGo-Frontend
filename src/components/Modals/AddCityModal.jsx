...
import { CreateCityApi } from "../../ApiRequests";
import { ErrorToast, SuccessToast } from "../../utils/ShowToast";
import { fetchCities } from "../../store/Slices/CitiesSlice";
import { useDispatch } from "react-redux";

const AddCityModal = ({ open, setOpen }) => {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await CreateCityApi({ name: CityName, shipping_fee: ShippingFee });
    ...
    <ModalWrapper open={open} setOpen={setOpen}>
      <div className="flex flex-col px-8 py-4">
        <div className="flex w-full justify-center items-center font-bold text-4xl border-b-[3px] border-b-[#0e2480] py-4 pb-6">
          <div className="text-3xl text-black">Add New City</div>
        </div>
